...
@st.cache_data
def read_azimuth(Delta):
    serie_data_1 = readd("data/Azimuth_Bow_Mancal_LA.txt")
    #lista_sample = []
    lista_sample, lista_data_x, lista_data_y, lista_data_z = axs(serie_data_1,Delta)
    
    sample = len(lista_data_x)

    T = sample * Delta                                      #período

    fs = 1 / Delta                                      # sample rate, Hz 

    return lista_sample, lista_data_x, lista_data_y, lista_data_z, sample, T, fs, Delta

    
    

@st.cache_data
def read_Compartiment(Delta):
    serie_data_1 = readd("data/Compartimento_57.txt")
    #lista_sample = []
    lista_sample, lista_data_x, lista_data_y, lista_data_z = axs(serie_data_1,Delta)
...
@st.cache_data
def read_Engine_01_Bottom(Delta):
    serie_data_1 = readd("data/Engine_01_Bottom_LA.txt")
    #lista_sample = []
    lista_sample, lista_data_x, lista_data_y, lista_data_z = axs(serie_data_1,Delta)
...
@st.cache_data
def read_Engine_01_top(Delta):
    serie_data_1 = readd("data/Engine_01_Top_LA.txt")
    #lista_sample = []
    lista_sample, lista_data_x, lista_data_y, lista_data_z = axs(serie_data_1,Delta)
...
@st.cache_data
def read_Turbo_Casing(Delta):
    serie_data_1 = readd("data/M01_Turbo_Casing.txt")
    #lista_sample = []
    lista_sample, lista_data_x, lista_data_y, lista_data_z = axs(serie_data_1,Delta)
...
@st.cache_data
def read_pipe_Tween(Delta):
    serie_data_1 = readd("data/Motor_1_Tubulação_Tween Deck.txt")
    #lista_sample = []
    lista_sample, lista_data_x, lista_data_y, lista_data_z = axs(serie_data_1,Delta)
...
def sideBar():
    ...
    with st.sidebar:
        selected=option_menu(
            ...
            options=["Azimuth Bow Mancal",
                    "Compartiment 57",
                    "Engine 01 bottom LA",
                    "Engine 01 Top LA",
                    "Turbo chasing",
                    "Motor 01 - Pipe Tween Deck"
                    ],
            icons=["house","eye","eye","eye","eye","eye"],
            menu_icon="cast",
            default_index=0
        )
    if selected=="Azimuth Bow Mancal":
        #st.subheader(f"Page: {selected}")
        with st.sidebar.expander("Delta"):
            Delta = st.number_input("Insert a Delta", value=0.000488, placeholder="Type a number...")              #dt
            st.write('dt is: ', Delta)
            st.divider()
        lista_sample, lista_data_x, lista_data_y, lista_data_z, sample, T, fs, Delta = read_azimuth(Delta)
        lista_v_x,lista_v_y,lista_v_z, lista_sample = cumtrp_vel_all(lista_sample, lista_data_x, lista_data_y, lista_data_z,fs)
    ...
    # ============ filter 2 ============= #
        ...
        fft_v_x, fft_v_y, fft_v_z, freq, Lin, nin = ftt_all(lista_v_x,lista_v_y,lista_v_z,fs)
        fig = all_figs(lista_data_x, lista_data_y, lista_data_z, lista_sample,lista_pos_x,lista_pos_y,lista_pos_z,lista_v_x,lista_v_y,lista_v_z,fs, fft_v_x, fft_v_y, fft_v_z, freq, Lin, nin,rms_v_x, rms_v_y, rms_v_z, rms_med)
    if selected=="Compartiment 57":
        #st.subheader(f"Page: {selected}")
        with st.sidebar.expander("Delta"):
            Delta = st.number_input("Insert a Delta", value=0.000488, placeholder="Type a number...")              #dt
            st.write('dt is: ', Delta)
            st.divider()
        lista_sample, lista_data_x, lista_data_y, lista_data_z, sample, T, fs, Delta = read_Compartiment(Delta)
        lista_v_x,lista_v_y,lista_v_z, lista_sample = cumtrp_vel_all(lista_sample, lista_data_x, lista_data_y, lista_data_z,fs)
        lista_pos_x, lista_pos_y, lista_pos_z = cumtrap_pos_all(lista_v_x,lista_v_y,lista_v_z,lista_sample)
    ...
    # ============ filter 2 ============= #
        ...
        with st.sidebar.expander("Filter 2"):
            ...
            second_filter_freq_1 = st.number_input(
                "Insert a Top Frequency", value=80, placeholder="Type a number...")   

            second_filter_freq_2 = st.number_input(
                "Insert a Low Frequency", value=1, placeholder="Type a number...")           
    ...
    if selected=="Engine 01 bottom LA":
        #st.subheader(f"Page: {selected}")
        with st.sidebar.expander("Delta"):
            Delta = st.number_input("Insert a Delta", value=0.000488, placeholder="Type a number...")              #dt
            st.write('dt is: ', Delta)
            st.divider()
        lista_sample, lista_data_x, lista_data_y, lista_data_z, sample, T, fs, Delta = read_Engine_01_Bottom(Delta)
        lista_v_x,lista_v_y,lista_v_z, lista_sample = cumtrp_vel_all(lista_sample, lista_data_x, lista_data_y, lista_data_z,fs)
        lista_pos_x, lista_pos_y, lista_pos_z = cumtrap_pos_all(lista_v_x,lista_v_y,lista_v_z,lista_sample)
    ...
    if selected=="Engine 01 Top LA":
        #st.subheader(f"Page: {selected}")
        with st.sidebar.expander("Delta"):
            Delta = st.number_input("Insert a Delta", value=0.000488, placeholder="Type a number...")              #dt
            st.write('dt is: ', Delta)
            st.divider()
        lista_sample, lista_data_x, lista_data_y, lista_data_z, sample, T, fs, Delta = read_Engine_01_top(Delta)
        lista_v_x,lista_v_y,lista_v_z, lista_sample = cumtrp_vel_all(lista_sample, lista_data_x, lista_data_y, lista_data_z,fs)
        lista_pos_x, lista_pos_y, lista_pos_z = cumtrap_pos_all(lista_v_x,lista_v_y,lista_v_z,lista_sample)
    ...
    if selected=="Turbo chasing":
        #st.subheader(f"Page: {selected}")
        with st.sidebar.expander("Delta"):
            Delta = st.number_input("Insert a Delta", value=0.000488, placeholder="Type a number...")              #dt
            st.write('dt is: ', Delta)
            st.divider()
        lista_sample, lista_data_x, lista_data_y, lista_data_z, sample, T, fs, Delta = read_Turbo_Casing(Delta)
        lista_v_x,lista_v_y,lista_v_z, lista_sample = cumtrp_vel_all(lista_sample, lista_data_x, lista_data_y, lista_data_z,fs)
        lista_pos_x, lista_pos_y, lista_pos_z = cumtrap_pos_all(lista_v_x,lista_v_y,lista_v_z,lista_sample)
    ...
    if selected=="Motor 01 - Pipe Tween Deck":
        #st.subheader(f"Page: {selected}")
        with st.sidebar.expander("Delta"):
            Delta = st.number_input("Insert a Delta", value=0.000488, placeholder="Type a number...")              #dt
            st.write('dt is: ', Delta)
            st.divider()
        lista_sample, lista_data_x, lista_data_y, lista_data_z, sample, T, fs, Delta = read_pipe_Tween(Delta)
        lista_v_x,lista_v_y,lista_v_z, lista_sample = cumtrp_vel_all(lista_sample, lista_data_x, lista_data_y, lista_data_z,fs)
        lista_pos_x, lista_pos_y, lista_pos_z = cumtrap_pos_all(lista_v_x,lista_v_y,lista_v_z,lista_sample)
    ...
    # ============ Medição ============= #
    with st.sidebar.expander("Measure"):
        sample = len(lista_data_x)
        st.write('The Sample is ', sample)

        T = sample * Delta                                      #período
        st.write('The Period is ', T)

        fs = 1 / Delta                                      # sample rate, Hz 
        st.write('The sample rate is ', fs)
